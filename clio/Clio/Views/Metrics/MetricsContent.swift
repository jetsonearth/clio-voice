...
struct MetricsContent: View {
    ...
    private var ioTrendChart: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("30-Day Io Trend")
                .font(.headline)
                .foregroundColor(DarkTheme.textPrimary)
    ...
    private var totalWordsTranscribed: Int {
        transcriptions.reduce(0) { $0 + TextUtils.countWords(in: $1.text) }
    }
    ...
    private var aiEnhancedCount: Int {
        transcriptions.filter { $0.isAIEnhanced }.count
    }
    
    private var averageASRLatency: TimeInterval {
        let relevantTranscriptions = transcriptions.filter { $0.processingLatencyMs != nil }
        guard !relevantTranscriptions.isEmpty else { return 0 }
        let totalLatency = relevantTranscriptions.reduce(0) { $0 + $1.ASRLatency }
        return totalLatency / Double(relevantTranscriptions.count)
    }
    
    private var averageLLMLatency: TimeInterval {
        let relevantTranscriptions = transcriptions.filter { $0.isAIEnhanced }
        guard !relevantTranscriptions.isEmpty else { return 0 }
        let totalLatency = relevantTranscriptions.reduce(0) { $0 + $1.LLMLatency }
        return totalLatency / Double(relevantTranscriptions.count)
    }
    
    private var averageTotalLatency: TimeInterval {
        let relevantTranscriptions = transcriptions.filter { $0.isAIEnhanced }
        guard !relevantTranscriptions.isEmpty else { return 0 }
        let totalLatency = relevantTranscriptions.reduce(0) { $0 + $1.totalLatency }
        return totalLatency / Double(relevantTranscriptions.count)
    }
}
