...
struct TimeEfficiencyView: View {
    ...
    private var headerSection: some View {
        VStack(alignment: .center, spacing: 8) {
            HStack(spacing: 8) {
                Text(localizationManager.localizedString("metrics.you_are"))
                    .font(.system(size: 32, weight: .bold))
                
                Text(String(format: localizationManager.localizedString("metrics.faster"), efficiencyMultiplierFormatted))
                    .font(.system(size: 32, weight: .bold))
                    .foregroundStyle(efficiencyGradient)
                
                Text(localizationManager.localizedString("metrics.with_voiceink"))
                    .font(.system(size: 32, weight: .bold))
            }
            .lineLimit(1)
    ...
    private var timeComparisonSection: some View {
        HStack(spacing: 16) {
            TimeBlockView(
                ...
                label: localizationManager.localizedString("metrics.speaking_time"),
            ...
            TimeBlockView(
                ...
                label: localizationManager.localizedString("metrics.typing_time"),
                icon: "keyboard.fill",
                color: .orange
            )
        }
    ...
    private var timeSavedView: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(localizationManager.localizedString("metrics.time_saved"))
    ...
    private var reportIssueButton: some View {
        Button(action: {
            ...
            HStack(alignment: .center, spacing: 12) {
                ...
                // Center text
                Text(localizationManager.localizedString("metrics.feedback_or_issues"))
                ...
                // Right button
                Text(localizationManager.localizedString("metrics.report"))
