...
class DictionaryManager: ObservableObject {
    ...
    private func loadItems() {
        guard let data = UserDefaults.standard.data(forKey: saveKey) else { 
            // No existing items - add default vocabulary for new users
            let defaultItem = DictionaryItem(word: "Clio")
            items = [defaultItem]
            saveItems()
            return 
        }
...
struct DictionaryView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            GroupBox {
                Label {
                    Text("Add words to help Clio recognize them properly(154 chars max, ~25 words). Works independently of AI enhancement.")
                        .font(.system(size: 12))
            ...
            HStack(spacing: 8) {
                ...
                AddNewButton("Add word", action: addWord, isEnabled: !newWord.isEmpty)
            }
