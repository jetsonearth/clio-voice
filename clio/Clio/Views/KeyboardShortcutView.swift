...
struct KeyboardShortcutView: View {
    ...
    private func shortcutComponents(from shortcut: KeyboardShortcuts.Shortcut) -> [String] {
        ...
        // Add modifiers
        if shortcut.modifiers.contains(.function) { components.append("Fn") }
    ...
    private func keyToString(_ key: KeyboardShortcuts.Key) -> String {
        ...
        case .nine: return "9"
        default:
            // Handle special keys by their raw values
            switch key.rawValue {
            case 6: return "Z"
            case 7: return "X"
            case 8: return "C"
...
struct KeyCapView: View {
    ...
    private var highlightGradient: LinearGradient {
        LinearGradient(
            colors: [
                DarkTheme.textPrimary.opacity(colorScheme == .dark ? 0.15 : 0.5),
                DarkTheme.textPrimary.opacity(0.0)
            ],
            startPoint: .topLeading,
            endPoint: .center
        )
    ...
    var body: some View {
        Text(text)
                ...
                ZStack {
                    ...
                    RoundedRectangle(cornerRadius: 8)
                        .strokeBorder(
                            LinearGradient(
                                colors: [
                                    DarkTheme.textPrimary.opacity(colorScheme == .dark ? 0.2 : 0.6),
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .stroke(
                        DarkTheme.textPrimary.opacity(colorScheme == .dark ? 0.1 : 0.3),
                        lineWidth: 1
                    )
                    .blur(radius: 1)
                    .offset(x: -1, y: -1)
