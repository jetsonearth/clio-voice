                ...
                if isLoading {
                    ...
                    if isHovering {
                        Text(formatTime(duration * Double(hoverLocation / geometry.size.width)))
                            ...
                            .padding(.vertical, 4)
                            .background(Capsule().fill(DarkTheme.textPrimary.opacity(0.2)))
    ...
    var body: some View {
        Capsule()
            .fill(
                LinearGradient(
                    colors: [
                        isPlayed ? Color.white : DarkTheme.textPrimary.opacity(0.3),
                        isPlayed ? DarkTheme.textPrimary.opacity(0.8) : DarkTheme.textPrimary.opacity(0.2)
                    ],
                    startPoint: .bottom,
                    endPoint: .top
                )
        ...
        VStack(spacing: 16) {
            HStack {
                HStack(spacing: 6) {
                    ...
                    Text(localizationManager.localizedString("audio_player.recording"))
            ...
            VStack(spacing: 16) {
                WaveformView(
                    ...
                    loadingText: localizationManager.localizedString("audio_player.generating_waveform"),
                ...
                HStack(spacing: 20) {
                    ...
                    .buttonStyle(.plain)
                    .help(localizationManager.localizedString("audio_player.show_in_finder"))
                    ...
                    .help(localizationManager.localizedString("audio_player.retranscribe_audio"))
            ...
            VStack {
                if showRetranscribeSuccess {
                    HStack(spacing: 8) {
                        ...
                        Text(localizationManager.localizedString("audio_player.retranscription_successful"))
                            .font(.system(size: 14, weight: .medium))
                    }
                    .padding(.horizontal, 16)
                    .padding(.vertical, 10)
                ...
                if showRetranscribeError {
                    HStack(spacing: 8) {
                        Image(systemName: "exclamationmark.circle.fill")
                            .foregroundColor(.red)
                        Text(errorMessage.isEmpty ? localizationManager.localizedString("audio_player.retranscription_failed") : errorMessage)
                            .font(.system(size: 14, weight: .medium))
                    }
                    .padding(.horizontal, 16)
                    .padding(.vertical, 10)
